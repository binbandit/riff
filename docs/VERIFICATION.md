...
- Swift protocol and transport: 5 tests passed, covering pairing-link validation, malformed links, Steam/deck persistence, starter sound references, and a live loopback HTTPS test of certificate pinning, invalid access keys, and redirect rejection. The loopback server is a test fixture, not the Windows companion.
...
- iPad app installed and launched in an iPad Pro 11-inch (M5) simulator. Its first screen was visually inspected. See [the actual simulator capture](screenshots/ipad-preview.png). It is an offline preview, not a simulated PC connection.

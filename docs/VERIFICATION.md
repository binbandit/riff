...
- Swift protocol, transport, and grid layout: 9 tests passed, covering pairing-link validation, malformed links, Steam/deck persistence, starter sound references, independent button duplication and long Unicode names, grid rotation, custom capacity in small windows, per-deck settings serialization, and a live loopback HTTPS test of certificate pinning, invalid access keys, and redirect rejection. The loopback server is a test fixture, not the Windows companion.

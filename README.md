...
For ready-made controls, open **+ → Add preset buttons**. Choose from 40 presets for music and volume, Windows, editing, browser tabs, game chat, and Riff controls. Search or filter by category, select several buttons, and tap **Add** to save them together in selection order. Selections stay checked when you change categories or search. For game-chat messages, choose your game's chat key before adding; the buttons open chat, type the message, and press Enter. You can edit the message, shortcut, timing, or appearance afterward. Presets can be added offline without AI setup. Running PC controls requires a connection with Soundboard mode turned off.
...
Tap the output label below the deck, or open **… → Sound controls**, to choose volume, output, and playback mode. **Overlap** layers effects; **One at a time** replaces the previous clip; **Queue** waits for each sound to finish before starting the next. Waiting buttons show their queue position. Open **Queue** beside the playback controls to drag sounds into a new order, remove individual entries, or **Clear queue** while the current sound keeps playing. The queue is also available from Sound controls. Tap a waiting button to remove it, or tap the playing button to skip to the next sound. Up to 48 sounds can wait in the queue. Switching to another mode clears waiting sounds on your next tap. **Stop all** stops sounds, clears the queue, and cancels any remaining sequence steps. Queue mode and queue editing require a compatible Windows companion and also work with offline iPad sounds; older companions show an update prompt for unavailable controls.

To repeat music or a sound clip, edit its button and enable **Loop sound**. A repeat badge marks looping buttons. Tap the button again or use **Stop all** to stop it. In Queue mode, the loop holds its place until stopped, then the next sound starts. Looping works offline on iPad and through both PC audio outputs with an updated companion.
